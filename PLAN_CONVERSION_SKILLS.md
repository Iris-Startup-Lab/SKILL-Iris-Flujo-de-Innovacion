...
| 7 | `Entrevistas de Empatia.md` | `entrevistas-empatia` | Diseño/Planeación | No (LLM-only, guía + plantilla) |
...
| 14 | `Dimensionador Estrategico de Ideas.md` | `dimensionador-ideas` | Análisis/Scoring | `calcular_score.py` (score compuesto /25 + consolidación multi-buyer persona) |
| 15 | `How Might We.md` | `how-might-we` | Facilitación | No (LLM-only) |
| 16 | `Ideacion.md` | `ideacion` | Facilitación/Generación | `evaluar_ideas.py` (cálculo de scores Novedad/Utilidad/Factibilidad) |
...
6. `persona-profile`
2. `entrevistas-empatia`
3. `problem-solution-fit` (+ `scripts/exportar_csv.py`)
4. `journey-builder`
5. `encuesta-kano` (+ `scripts/clasificar_kano.py`, `references/`)
6. `discovery-survey` (+ `scripts/calcular_muestra.py`, `references/`)
7. `day-in-the-life`
8. `expo-quest`

### Prioridad 4 — Fase 3: Ideación (6 skills)
 1. `how-might-we` (+ `references/matriz-ambicion-palancas.md`)
 2. `ideacion` (+ `scripts/evaluar_ideas.py`, `references/`)
 3. `caressing-client`
 4. `referral-builder`
 5. `business-model-navigator` (+ `references/catalogo-patrones.md`)
 6. `dimensionador-ideas` (+ `scripts/calcular_score.py`, `references/`)

### Prioridad 5 — Fase 4-5: Prototipado y Validación (7 skills)
 1. `landing-page`
 2. `landing-ux-analyzer`
 3. `online-ads`
 4. `email-campaign` (+ `scripts/calcular_significancia.py`)
 5. `explainer-video`
 6. `feature-stub`
 7. `popup-store`

### Prioridad 6 — Macro-Skill orquestadora
 1. `macro_skill_flujo_de_innovacion_iris/SKILL.md` — skill principal
