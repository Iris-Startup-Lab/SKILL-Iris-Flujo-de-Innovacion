...
SKILL.md sobre los JSON de entrada (Fases 1-3): toda señal con
...
def validar_invariante_json(json_paths):
    """Invariantes de SKILL.md sobre los JSON de Fases 1-3:
            ...
            for item in bloque.get(items_key, []):
                ...
                if clasif in ("confirmacion", "tension") and escala is True:
                    errores.append(ValidationError(
                        ...
                        f"(Filtro 2 de SKILL.md: no escala como señal débil)"
                    ))
