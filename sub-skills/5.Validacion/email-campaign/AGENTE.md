...
Esta skill tiene **dos scripts y no hacen lo mismo**: `calcular_significancia.py` se corre
**antes**, para saber qué muestra hace falta; `analizar_resultados.py` se corre **después**, con
los datos en la mano. La fórmula de muestra por variante es la misma en los dos, así que no se
contradicen.

El flujo diseñaba los experimentos pero no sabía leerlos: «clic: 37 de 420» se comparaba
...
python sub-skills/5.Validacion/email-campaign/scripts/analizar_resultados.py \
    --k 37 --n 420 --umbral 0.06 --metrica "clic" \
...
python sub-skills/5.Validacion/email-campaign/scripts/analizar_resultados.py \
    --k 37 --n 420 --control-k 12 --control-n 400 --metrica "clic"
```
...
concluir— **cuántos correos entregados más harían falta**. Con `--datos` acepta varias variantes a la vez.
